<?php
    ...
    if ($option == "listregistros"){
        ...
        if (!empty($arrPersona)){
            for ($i=0; $i < count($arrPersona) ; $i++) {
                ...
                $options = '<a href="'.BASE_URL.'views/persona/editar-persona.php?p='.$idpersona.'"
                class="btn btn-outline-primary btn-sm"
                title="Editar registro">
                <i class="fas fa-user-edit"></i></a>
                <button class="btn btn-outline-danger btn-sm" title="Eliminar registro"
                onclick="fntDelPersona('.$idpersona.')" >
    ...
    if ($option == "registro") {
        if ($_POST) {
            ...
            $arrPersona = $objPersona->insetPersona($strNombre, $strApellido, $intTelefono, $strEmail);
            if ($arrPersona->id > 0) {
                $arrResponse = array('status' => true, 'msg' => 'Datos guardados correctamente');
            }else {
                $arrResponse = array('status' => false, 'msg' => 'Error al guardar o Correo ya existente');
                }
            }
            echo json_encode($arrResponse);
        }
    ...
    if ($option == "actualizar") {
        if ($_POST) {
            ...
            if ($arrPersona->idp > 0) {
                ...
                $arrResponse = array('status' => false, 'msg' => 'Error al actualizar o Correo ya existente');
                }
            }
            echo json_encode($arrResponse);
        }
